...
3. Distribution of components in succes index formula

```{r}
plot_histogram(result[2:15])
```

4. Pairwise correlation between these components

```{r}
plot_correlation(result[2:15])
```

5. Performance Evaluation between models
...
8. Contributions of Variables on top 5 PCs
...
```{r}
P_result <- prcomp(Res[2:15], scale = TRUE)
fviz_eig(P_result, addlabels = TRUE)
```

10. Contributions of Variables on top 4 PCs
